...
---
<!-- Vendored from canary/SKILL.md @ e76f65a8da31ec14776a965c608222c1aecad656; adaptations logged in ../../VENDOR.md. MIT (c) 2026 Garry Tan. -->

**Tool scope (upstream frontmatter, restated).** Upstream restricted this skill to these tools via an `allowed-tools` frontmatter key, not part of the open Agent Skills standard: Bash, Read, Write, Glob, AskUserQuestion. Where your harness supports scoping a skill's tool access, scope it the same way; otherwise treat this as the set of tools the skill actually exercises.
...
When running inside this platform (agent-platform), this is the post-deploy monitoring half of the F.9 release stage, after the merge gates have already cleared, not a way to skip them. Land alert screenshots and health-check evidence under `.agent/runs/<TASK-ID>/verification/` alongside the rest of the F.7 evidence bundle for the release, so a regression this skill catches is traceable back to the same run directory as the original verification. A canary alert that finds a regression re-opens the task; it never authorizes a direct hotfix push around the merge queue.
