...
---
<!-- Vendored from investigate/SKILL.md @ e76f65a8da31ec14776a965c608222c1aecad656; adaptations logged in ../../VENDOR.md. MIT (c) 2026 Garry Tan. -->

**Tool scope (upstream frontmatter, restated).** Upstream restricted this skill to these tools via an `allowed-tools` frontmatter key, not part of the open Agent Skills standard: Bash, Read, Write, Edit, Grep, Glob, AskUserQuestion, WebSearch. Where your harness supports scoping a skill's tool access, scope it the same way; otherwise treat this as the set of tools the skill actually exercises.
**Hook-based enforcement (upstream frontmatter, restated).** Upstream wired a `PreToolUse` hook (on `Edit`/`Write`) to a `check-freeze.sh` script that blocks edits outside the debug-scope boundary recorded below, not part of the open Agent Skills standard. Where your harness supports pre-tool-use hooks, wire the same check; otherwise apply this discipline yourself: before every edit or write during an investigation, check the recorded scope-lock boundary (`freeze-dir.txt` per the Scope lock section below) and refuse edits outside it.
