...
- qa the ios app
---
<!-- Vendored from ios-qa/SKILL.md @ e76f65a8da31ec14776a965c608222c1aecad656; adaptations logged in ../../VENDOR.md. MIT (c) 2026 Garry Tan. -->

**Tool scope (upstream frontmatter, restated).** Upstream restricted this skill to these tools via an `allowed-tools` frontmatter key, not part of the open Agent Skills standard: Bash, Read, Write, Edit, Grep, Glob, AskUserQuestion. Where your harness supports scoping a skill's tool access, scope it the same way; otherwise treat this as the set of tools the skill actually exercises.
...
When running inside this platform (agent-platform), this is the spec §9.3 iOS verification binding for F.7. Land the reproduce → fix → rebuild → reverify evidence (screenshots, StateServer captures, the specific repro steps) under `.agent/runs/<TASK-ID>/verification/`, shaped as `verification/result.json` entries per the F.7 result shape, so a human or the independent verifier can see exactly what was driven on the device and what it showed. As with `qa`, this skill's own live-device pass does not replace F.7's independent, re-executed verification of the candidate on the same device — it feeds evidence into that gate, it does not certify the candidate itself.
