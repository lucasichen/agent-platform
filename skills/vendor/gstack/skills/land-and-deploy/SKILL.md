...
- ship to production
---
<!-- Vendored from land-and-deploy/SKILL.md @ e76f65a8da31ec14776a965c608222c1aecad656; adaptations logged in ../../VENDOR.md. MIT (c) 2026 Garry Tan. -->
...
When running inside this platform (agent-platform), this workflow maps onto the F.9 merge/release stage and must go through it, not around it. The merge step here is the platform's merge queue, gated on independent F.7 verification and F.8 review having already passed for this candidate — never a direct merge that bypasses those gates because this skill's own pre-merge checks came back green. Its own single-pass verification is a pre-merge sanity check, not a substitute for F.7's independent run.
