...
- test the site
---
<!-- Vendored from qa/SKILL.md @ e76f65a8da31ec14776a965c608222c1aecad656; adaptations logged in ../../VENDOR.md. MIT (c) 2026 Garry Tan. -->

**Tool scope (upstream frontmatter, restated).** Upstream restricted this skill to these tools via an `allowed-tools` frontmatter key, not part of the open Agent Skills standard: Bash, Read, Write, Edit, Glob, Grep, AskUserQuestion, WebSearch. Where your harness supports scoping a skill's tool access, scope it the same way; otherwise treat this as the set of tools the skill actually exercises.
...
When running inside this platform (agent-platform), route this skill's output into the F.7 verifier's evidence bundle rather than an ad hoc location: write before/after health scores, per-bug fix evidence, and the ship-readiness summary under `.agent/runs/<TASK-ID>/verification/`, shaped as `verification/result.json` entries (one per check: name, PASS/FAIL status, an evidence pointer — screenshot, trace, or log) per the F.7 result shape. A health-score improvement with no attached evidence is not a PASS under that contract. This skill finding and fixing bugs is not a substitute for F.7's independent, re-executed verification of the same candidate — it runs before or alongside that gate, never instead of it.
