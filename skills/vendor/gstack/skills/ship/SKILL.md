...
- deploy this
---
<!-- Vendored from ship/SKILL.md @ e76f65a8da31ec14776a965c608222c1aecad656; adaptations logged in ../../VENDOR.md. MIT (c) 2026 Garry Tan. -->

**Tool scope (upstream frontmatter, restated).** Upstream restricted this skill to these tools via an `allowed-tools` frontmatter key, not part of the open Agent Skills standard: Bash, Read, Write, Edit, Grep, Glob, Agent, AskUserQuestion, WebSearch. Where your harness supports scoping a skill's tool access, scope it the same way; otherwise treat this as the set of tools the skill actually exercises.
...
When running inside this platform (agent-platform), this workflow maps onto the F.9 merge/release stage, not around it. Opening the PR is where this skill's authority ends: the platform's merge queue and merge gates (independent F.7 verification, F.8 review) still have to clear before anything lands, exactly as they would for any other candidate. Never let this skill's own test run, diff review, or "green" signal stand in for those independent gates, and never push directly to a protected branch as a shortcut around them.
