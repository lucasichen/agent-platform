...
---
<!-- Vendored from skills/engineering/ask-matt/SKILL.md @ 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76; adaptations logged in ../../VENDOR.md. MIT (c) 2026 Matt Pocock. -->
...
Named skills above (`/grill-with-docs`, `/prototype`, `/to-spec`, `/to-tickets`, `/code-review`, `/diagnosing-bugs`, `/wayfinder`, `/improve-codebase-architecture`, `/domain-modeling`, `/research`) map to their `pocock-*` counterparts vendored in this pack; skills named here that were not adopted (`/implement`, `/tdd`, `/triage`, `/codebase-design`, `/grill-me`, `/grilling`, `/resolving-merge-conflicts`, `/to-questionnaire`, `/wizard`, `/wait-what`, `/teach`, `/writing-for-agents`, `/handoff`, `/setup-matt-pocock-skills`) are not vendored on this platform — treat every reference to them below as upstream methodology context for the flow's shape, not an instruction to invoke a skill that does not exist here. **This skill is advisory only and never owns routing.** It may recommend which vendored skill or platform role fits a situation (per spec §4's planning pipeline: F.1 investigation, F.1A domain/product, F.2 specification, F.3 architecture, F.4 decomposition), but F.0 (the control plane / mission compiler) is the sole owner of actual task routing, workflow binding, and role assignment. Where this skill's flow diverges from the platform's own `roles/*.md` contracts or `policies/bindings.yaml`, the platform contract wins; use this skill to explain the *why* of a step, never to override which role or gate governs it.
